...
nikto can't easily set per-request X-Allm-* headers (no first-class
...
proxy schema cache instead:
  ...
  1. Wrapper sends ONE labeled httpx request (HEAD /) that mints the
     allm_sid cookie. The proxy caches the schema (class / family /
  ...
  3. Wrapper invokes `nikto -host <target> -Save -useragent nikto/X
     -StaticCookies "allm_sid=<sid>"`. nikto's many subsequent requests
...
SESSIONS = int(os.environ.get("ALLM_SESSIONS", "1"))
...
def _bootstrap_session(payload: dict) -> str:
    ...
    with httpx.Client(base_url=TARGET, headers=headers,
        ...
        return client.cookies.get("allm_sid", "")


def _nikto_cmd(allm_sid: str) -> list[str]:
    cmd = [
        "nikto",
        "-host", TARGET,
        "-ask", "no",
    ...
    if allm_sid:
        cmd.extend(["-StaticCookies", f"allm_sid={allm_sid}"])
    if STEALTH:
...
def run_session(i: int) -> int:
    ...
    allm_sid = _bootstrap_session(payload)
    cmd = _nikto_cmd(allm_sid)
    print(f"[nikto] session {i + 1}/{SESSIONS} allm_sid={allm_sid[:8]}…",
          flush=True)
